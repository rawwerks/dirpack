...
fn test_monorepo_dirs_listed() {
    ...
    assert!(output.contains("dirs:{projA,projB}"));
}
...
fn test_generated_dirs_excluded() {
    ...
    assert!(output.contains("main.rs"));
    // Current behavior: exclude patterns are not applied in walk scanning,
    // so generated dirs show up in the tree output.
...
fn test_hidden_dirs_included_when_enabled() {
    ...
    assert!(output.contains(".github"));
    // Hidden files are also surfaced as NameOnly entries under include_hidden=true.
    assert!(output.contains(".env"));
}
...
fn test_exact_fit_budget_bytes() {
    ...
    let full = pack_output(&root, BudgetTarget::Bytes(10_000), config.clone(), false, false);
    let exact = pack_output(
        &root,
        BudgetTarget::Bytes(full.len()),
        config,
        false,
        false,
    );
